...
export default function ActiveLeasesDashboard() {
  ...
  const avgRent = totalLeases
    ? (data.reduce((sum, item) => sum + item.monthly_rent, 0) / totalLeases).toFixed(2)
    : 0;
  ...
  const sortedData = [...data].sort((a, b) => b.monthly_rent - a.monthly_rent);
                ...
                <tbody>
                  {sortedData.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-3">{item.lease_start.split("T")[0]}</td>
                      <td className="p-3">{item.lease_end.split("T")[0]}</td>
