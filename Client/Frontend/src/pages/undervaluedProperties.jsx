...
import { TrendingDown, PieChart, Home, MapPin, IndianRupee } from "lucide-react";
import '../App.css';
import '../index.css';
    ...
    return (
      <div className="bg-white p-4 border border-gray-200 rounded-md shadow-lg">
        ...
        <p className="text-teal-600 font-medium flex items-center">
          ...
          Undervaluation: ₹{payload[0].value.toFixed(2).toLocaleString()}
        </p>
      </div>
    );
  }
...
export default function UndervaluedProperties() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setIsLoading(true);
        const response = await fetch("http://127.0.0.1:8000/undervalued-properties");
  ...
  const avgDelta = totalProperties
    ? (data.reduce((sum, item) => sum + item.delta, 0) / totalProperties).toFixed(2)
    : 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-teal-50">
      <div className="max-w-6xl mx-auto p-6">
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 bg-white p-6 rounded-lg shadow-md border-t-4 border-teal-600">
          <div>
            <h1 className="text-3xl font-bold text-teal-700 flex items-center">
              <TrendingDown className="mr-3 text-teal-600" />
              Undervalued Properties Dashboard
            </h1>
            <p className="text-gray-500 mt-1">Properties priced below market average</p>
          </div>
          <div className="mt-4 md:mt-0 bg-teal-50 px-4 py-2 rounded-full text-teal-700 text-sm font-medium">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-teal-600 hover:shadow-lg transition-shadow">
            <div className="flex items-center">
              <div className="bg-teal-100 p-3 rounded-full mr-4">
                <Home size={24} className="text-teal-600" />
              </div>
              <div>
                <p className="text-sm text-gray-500 uppercase tracking-wider">Undervalued Properties</p>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-teal-600 hover:shadow-lg transition-shadow">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm text-gray-500 uppercase tracking-wider">Avg. Price Gap</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden order-2 lg:order-1">
            <div className="bg-gradient-to-r from-teal-600 to-teal-500 text-white p-4 font-semibold flex items-center">
              <PieChart size={20} className="mr-2" />
              Undervaluation by Neighborhood
            </div>
            <div className="p-4">
              ...
              <table className="w-full text-left">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>
                    ...
                    <th className="p-3 text-gray-600 font-semibold text-right">Undervaluation</th>
                  </tr>
                </thead>
                <tbody>
                  {sortedData.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-3 font-medium text-teal-700 text-right">
                        ₹{item.delta.toFixed(2).toLocaleString()}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="mt-8 text-center text-gray-500 text-sm p-4 bg-white rounded-lg shadow-md">
          <p>Made with ❤️ | Undervalued Properties Dashboard</p>
        </div>
      </div>
    </div>
  );
